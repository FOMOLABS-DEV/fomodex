...
import { X, Shield, Plus, Trash2, Loader2, AlertCircle, LogOut, Eye, EyeOff, Lock, Globe, FileText, Zap, Twitter, MessageCircle, Mail, Archive, RotateCcw } from 'lucide-react'
...
export function AdminPanel({ isOpen, onClose }: AdminPanelProps) {
  ...
  return (
    <AnimatePresence>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/90 backdrop-blur-xl flex items-center justify-center z-[10000] p-2 sm:p-4" onClick={onClose}>
        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-[#0a0a0f] border border-[#1a1a2e] rounded-[24px] sm:rounded-[32px] w-full max-w-4xl max-h-[95vh] overflow-hidden shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
          <div className="p-4 sm:p-8 border-b border-[#1a1a2e] flex items-center justify-between bg-[#0d0d15]">
            <div className="flex items-center gap-3 sm:gap-5">
              <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center"><Shield className="w-5 h-5 sm:w-7 sm:h-7 text-black" /></div>
              <div><h2 className="text-lg sm:text-2xl font-black tracking-tighter uppercase">Admin Panel</h2><p className="text-[8px] sm:text-xs text-gray-500 font-bold uppercase tracking-widest">Management</p></div>
            </div>
            <button onClick={onClose} className="w-8 h-8 sm:w-12 sm:h-12 flex items-center justify-center bg-[#11111a] border border-[#1a1a2e] rounded-lg sm:rounded-2xl text-gray-400 hover:text-white"><X className="w-4 h-4 sm:w-6 sm:h-6" /></button>
          </div>

          <div className="p-4 sm:p-8 overflow-y-auto custom-scrollbar">
            {!isLoggedIn ? (
              <div className="max-w-md mx-auto py-8 sm:py-12">
                <div className="text-center mb-8"><div className="w-16 h-16 sm:w-20 sm:h-20 mx-auto rounded-2xl bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center mb-4"><Lock className="w-8 h-8 sm:w-10 sm:h-10 text-cyan-400" /></div><h3 className="text-xl sm:text-2xl font-black uppercase">Authenticate</h3></div>
                <div className="space-y-4">
                  <div><label className="text-[9px] font-black text-gray-600 uppercase tracking-widest">Username</label><input type="text" value={username} onChange={e => setUsername(e.target.value)} placeholder="Identifier" className="w-full bg-[#11111a] border border-[#1a1a2e] rounded-xl px-4 py-3 outline-none focus:border-cyan-500/50 text-sm" onKeyDown={e => e.key === 'Enter' && handleLogin()} /></div>
                  <div><label className="text-[9px] font-black text-gray-600 uppercase tracking-widest">Password</label><div className="relative"><input type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder="Key" className="w-full bg-[#11111a] border border-[#1a1a2e] rounded-xl px-4 py-3 outline-none focus:border-cyan-500/50 text-sm" onKeyDown={e => e.key === 'Enter' && handleLogin()} /><button onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-600">{showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}</button></div></div>
              ...
              <div className="space-y-8">
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-[#11111a] p-3 rounded-xl border border-[#1a1a2e]">
                  <div className="flex items-center gap-3"><div className="w-8 h-8 rounded-full bg-cyan-500/20 flex items-center justify-center text-cyan-400 font-black text-xs">{username.charAt(0)}</div><div><div className="text-[8px] font-black text-gray-600 uppercase">Operator</div><div className="text-xs font-black text-cyan-400 uppercase">{username}</div></div></div>
                  <div className="flex items-center gap-1.5 p-1 bg-[#0a0a0f] rounded-lg border border-[#1a1a2e] w-full sm:w-auto overflow-x-auto">
                    ...
                    <div className="w-px h-4 bg-[#1a1a2e] mx-1" /><button onClick={handleLogout} className="p-1.5 text-gray-500 hover:text-red-400"><LogOut className="w-3.5 h-3.5" /></button>
                  </div>
                </div>
